...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract 1000nm only-tpp dataset from legacy full-spectrum npz.")
    ...
    data = np.load(src)
    required = {"structures", "tpp_mag", "lambdas", "thetas"}
    ...
    tpp_row = tpp[:, lam_idx, :]
    valid = np.isfinite(tpp_row).all(axis=1)
