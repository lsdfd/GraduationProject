...
def eval_one_case(
    ...
    cond_raw: np.ndarray,       # [2,11,17]   物理空间条件（评估 MAE 用）
    n_samples: int,
    device: str,
    lambdas: np.ndarray,
    thetas: np.ndarray,
    ...
    for i, s in enumerate(structs):
        st = torch.from_numpy(s).unsqueeze(0).unsqueeze(0).float()  # [1,1,64,64]
        cond_pred = rcwa_eval_full_map(st, cond_raw.shape[0], device)
        if cond_pred is None:
            # RCWA 不可用时用 placeholder
            scores.append(-1.0)
            continue
        ...
        if sc > best_score:
            best_score     = sc
            best_idx       = i
            best_pred_cond = cond_pred

    if best_pred_cond is None:
        best_pred_cond = np.zeros_like(cond_raw)
