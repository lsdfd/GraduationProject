# -*- coding: utf-8 -*-
"""
900 nm 波段 k 空间传递函数扫描入口。

将 --lambda_nm 默认值设为 900，输出文件存放在本目录，
其余逻辑完全复用顶层 imag_process/scan_kspace.py。

Usage
-----
  python src/infer/band_900nm/imag_process/scan_kspace.py --sample 0
  python src/infer/band_900nm/imag_process/scan_kspace.py --from_infer
  python src/infer/band_900nm/imag_process/scan_kspace.py --sample 0 --run_phi45
"""
...
_DEFAULT_LAMBDA_NM = 900.0


def main():
    parser = argparse.ArgumentParser(description="k 空间传递函数 2D 图 @ 900 nm")
    ...
    if args.from_infer:
        if args.infer_dir is None:
            laplas_root = _ROOT / "samples" / "laplas_900nm"
            runs = sorted(laplas_root.iterdir()) if laplas_root.exists() else []
            if not runs:
                laplas_root = _ROOT / "samples" / "laplas"
                runs = sorted(laplas_root.iterdir()) if laplas_root.exists() else []
            if not runs:
    ...
    from infer.imag_process.scan_kspace import NA  # noqa: E402
