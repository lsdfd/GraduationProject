...
将 --lambda_nm 默认值设为 900，其余逻辑完全复用顶层 imag_process/image_processing.py。

Usage
-----
  python src/infer/band_900nm/imag_process/image_processing.py
  python src/infer/band_900nm/imag_process/image_processing.py --pol x --from_infer
  python src/infer/band_900nm/imag_process/scan_kspace.py
"""
...
_IMAG_PROCESS_DIR = _HERE  # kspace_result.npz 默认存放在本目录


def main():
    parser = argparse.ArgumentParser(description="Fourier optics imaging simulation @ 900 nm")
