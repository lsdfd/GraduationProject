...
def load_target_cond(target_path, stats_path, device):
    target = np.load(target_path).astype(np.float32)   # [C,11,13], C order: [tpp_mag, tss_mag]
...
def main():
    ...
    cond_batch = target_cond.repeat(cfg["num_samples"], 1, 1, 1)   # [K,C,11,13]
    samples = diffusion.sample(cond_batch, cfg_scale=cfg["cfg_scale"])  # [K,1,64,64]
    pred_cond = surrogate(samples)                                     # [K,C,11,13]
