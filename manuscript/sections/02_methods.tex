...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{figures/assets/figure1_framework.png}
\caption{Overview of the physics-consistent AFOM inverse-design framework. A target wavelength--angle--polarization response tensor drives simulation-grounded dataset construction, forward-surrogate training, conditional diffusion-based inverse design, surrogate-assisted candidate selection, and final output verification across unified AFOM functional families.}
\label{fig:framework}
\end{figure}

Figure~\ref{fig:framework} summarizes this closed-loop pipeline from target response specification to candidate generation and final task-specific output verification. In symbolic form, the workflow can be written as
\begin{equation}
